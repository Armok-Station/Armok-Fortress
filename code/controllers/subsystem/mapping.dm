#define INIT_ANNOUNCE(X) to_chat(world, span_boldannounce("[X]"), MESSAGE_TYPE_DEBUG); log_world(X) // TGP ADDITION
	...
	*/
	...
	if(CONFIG_GET(flag/persistent_save_enabled) && persistent_save_z_levels[ZTRAIT_ICE_RUINS] && SSworld_save.map_configs_cache?[ZTRAIT_ICE_RUINS])
		for(var/datum/map_config/persistent_map in SSworld_save.map_configs_cache[ZTRAIT_ICE_RUINS])
			...
			LoadGroup(FailedZs, persistent_map.map_name, persistent_map.map_path, persistent_map.map_file, persistent_map.traits, null, height_autosetup = persistent_map.height_autosetup)
	// TGP EDIT ADDITION END
	else if(current_map.wilderness_levels) // TGP EDIT
...
//#define INIT_ANNOUNCE(X) to_chat(world, span_boldannounce("[X]"), MESSAGE_TYPE_DEBUG); log_world(X) TGP EDIT - Moved to top of file
...
/datum/controller/subsystem/mapping/proc/loadWorld()
	...
	*/
	...
	if(LAZYLEN(FailedZs)) //but seriously, unless the server's filesystem is messed up this will never happen
		...
		msg += ". Yell at your server host!"
		INIT_ANNOUNCE(msg)
//#undef INIT_ANNOUNCE TGM EDIT - Moved to bottom of file
...
#undef INIT_ANNOUNCE // TGM EDIT - Moved this to bottom
